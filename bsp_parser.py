...
def table_mapping(engine):
    ...
    with Session() as session:
        table_mapping_query = text("SELECT src_id, src_table, parsed_data from iata_metadata_bsp.source_file_info")
...
def fetch_metadata_for_table(engine, table_name):
    metadata_query = text(f"SELECT db_element_name, start_pstn, element_lgth, data_type FROM iata_metadata_bsp.{table_name}")
    try:
...
if __name__ == "__main__":
    ...
    subfolders_to_db = {'BSP': 'prd_bsp_files'}
